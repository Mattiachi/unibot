...
def handle(chat_id,message):
	...
	if(state == 4): #Inserendo anno di frequentazione
		if(message.isdigit()):
			...
			tastiera = ["Oggi","/oggi","Domani","/domani","Menù", "/menu"]
			bot.sendMessage(chat_id, "Registrazione completata! Prova le funzionalità'", reply_markup=makeKeyboard(tastiera))
			upgradeStato(chat_id)
			return
		else:
	...
	if(state == 6 or state == 7):
		...
		if(state == 6):
			...
			deleteSchedule(chat_id, "/oggi") #Vengono cancellati eventuali presenti jobs che non devono piu essere eseguiti
